...
- enum-like config types such as `LogLevel` and `LogFormat`
- defaults application
- struct-level validation via `Validate()`
- type-level parsing/validation via `UnmarshalText`
- the shared decode hook used by the current `viper`/`mapstructure` pipeline

This package is intentionally optimized for reference clarity rather than for the strictest possible layer separation.
...
- keeping validation-related practices in one place makes them easier to study, reuse, and simplify
